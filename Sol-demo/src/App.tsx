...
export default function App() {
  ...
  useEffect(() => {
    ...
    /**
   * @description airdrops 2 SOL into Sender Wallet.
   * This function is called when the AirDrop SOL button is clicked
   */
    const airDropSol = async () => {
      // request airdrop into this new account
      const myWallet = await Keypair.fromSecretKey(senderKeypair!.secretKey);
  ...
  const transferSol = async () => {  
    const senderBalanceBeforeTransfer=await connection.getBalance(senderKeypair!.publicKey) / LAMPORTS_PER_SOL;
    const recieverBalanceBeforeTransfer=await connection.getBalance(receiverPublicKey!) / LAMPORTS_PER_SOL;
    console.log("Sender Balance before Transaction: " + senderBalanceBeforeTransfer);  
    console.log("Receiver Balance Before Transaction: " + recieverBalanceBeforeTransfer);
    const transferAmountWithGasFee=1+.000005;
    if (senderBalanceBeforeTransfer<transferAmountWithGasFee){
      console.log("the sender doesn't have sufficienct amount in wallet to transfer");
    }
